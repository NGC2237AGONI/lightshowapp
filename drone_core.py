...
class DataExtractor:
    ...
    def run(self, input_file, scale):
        ...
        return True, f"提取完成！共 {len(self.all_x)} 个点。"
...
class FormationOptimizer:
    def run(self, axis_mode, target_count, safety_distance):
        try:
            ...
            if len(pts) == 0: return False, "加载 model_data.npz 失败", None, None
            ...
            return True, f"优化完成，最终点数: {len(fin_pts)}", fin_pts, fin_cols
    ...
    def _finalize_drone_layout(self, curr_pts, curr_cols, curr_nms, curr_ids, src_pts, src_cols, src_nms, src_ids, target_count, min_dist):
        ...
        if diff == 0: return curr_pts, curr_cols, curr_nms, curr_ids
        elif diff < 0:
            ...
            if num_to_add == 0: return curr_pts, curr_cols, curr_nms, curr_ids
...
class AnimationExporter:
    ...
    def run_export(self, fbx_file, anim_index, custom_duration, fps, scale, axis_mode, static_threshold, end_buffer):
        ...
        try:
            with open("drone_path.csv", 'w', newline='', encoding='utf-8') as f:
                ...
                root = scene.GetRootNode()
                if root:
                    self._process_node_smart(root, start, max_frames, fps, scale, axis_mode, static_threshold, end_buffer, writer)
        except Exception as e:
    ...
    def _process_node_smart(self, node, start_time, max_frames, fps, scale, axis_mode, static_thresh, end_buffer, writer):
        attr = node.GetNodeAttribute()
        if attr and attr.GetAttributeType() == fbx.FbxNodeAttribute.EType.eMesh:
            mesh_name = node.GetName()
            if mesh_name in self.TARGET_MAP:
                self._extract_and_prune_data(node, mesh_name, start_time, max_frames, fps, scale, axis_mode, static_thresh, end_buffer, writer)
        for i in range(node.GetChildCount()):
            self._process_node_smart(node.GetChild(i), start_time, max_frames, fps, scale, axis_mode, static_thresh, end_buffer, writer)

    def _extract_and_prune_data(self, node, mesh_name, start_time, max_frames, fps, scale, axis_mode, static_thresh, end_buffer, writer):
        mesh = node.GetMesh()
...
class TrajectoryOptimizer:
    def optimize_trajectory(self, csv_file, safe_dist, max_vel, bound_L, bound_W, bound_H, manual_time_scale=None):
        try:
            ...
            # 1. 空间缩放 (Spatial Scaling)
            # 目标: MinDistance >= SafeDist
            ...
            if min_dist_original < safe_dist:
                spatial_scale = safe_dist / min_dist_original * 1.05 # 留5%余量
            ...
            # 2. 时间缩放 (Time Scaling)
            # 目标: MaxVelocity <= MaxVelLimit
            # 空间放大了，路程变长，为了保持速度不超标，时间也得拉长（慢放）
